...
public class KnucklesPlayerController : MonoBehaviour
{
    ...
    public void CollectRing()
    {
        ...
        //ringPickup.Play();
        PlayRingSound();
        AudioSource.PlayClipAtPoint(ringPickup.clip, transform.position);
    }
        ...
        private void PlayRingSound()
    {
        ...
        tempAudio.clip =  ringPickup.clip;
        tempAudio.volume =  ringPickup.volume;
        tempAudio.pitch =  ringPickup.pitch;
        tempAudio.spatialBlend =  ringPickup.spatialBlend;
    ...
    private IEnumerator Hit()
    {
        ...
        animator.SetBool("hitAnimation", hitCollider);
...

    {
        ...
        public void TakeDamage2(Vector2 sourcePosition)

    {
        if (invFrame) return;
        if (isDead) return;

        StartCoroutine(HandleDamageKnockback2(sourcePosition));
    }
    ...
    //SONIC CHARGING FUNCTIONALITY, ISN'T NEEDED FOR KNUCKLES, YET.
    ...
    private IEnumerator HandleDamageKnockback2(Vector2 sourcePosition)
    {
        isDead = true;
        invFrame = true;
    ...
    void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.Z) && isGrounded && !hitCollider.enabled)
        {
            StartCoroutine(Hit());
        }
